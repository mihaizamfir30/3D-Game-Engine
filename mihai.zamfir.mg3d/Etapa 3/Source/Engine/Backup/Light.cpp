...
Light::Light(glm::vec3 lightPosition, glm::vec3 lightColor) {
	...
	mesh = ResourceManager::GetInstance()->LoadModel(0);
	
	shader = ResourceManager::GetInstance()->LoadShader(0);
...
void Light::Update(double deltaTime) {
	...
	lightPosition = glm::vec3(0.0, 0.0f, 0.0f);
	lightPosition.x = cos(time) * radius;
	lightPosition.z = sin(time) * radius;

	transform->SetPosition(lightPosition);
